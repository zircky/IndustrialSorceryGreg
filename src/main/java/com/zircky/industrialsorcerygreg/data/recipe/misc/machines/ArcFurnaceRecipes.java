...
//        .save(provider);
    ...
    ARC_FURNACE_RECIPES.recipeBuilder("piled_tbcc_oxygen_to_tbcco").duration(240).EUt(30720, 3)
        .inputDust(PiledTBCC)
        .inputFluids(Oxygen.getFluid(1000))
        .outputDust(TBCCO)
        .save(provider);
    ...
    // Os + 4O -> OsO4
    ARC_FURNACE_RECIPES.recipeBuilder("osmium_oxygen_to_osmium_tetroxide").duration(520).EUt(4800, 3)
        .inputDust(Osmium)
        .inputFluids(Oxygen.getFluid(4000))
        .outputDust(OsmiumTetroxide, 5)
        .save(provider);
  }
